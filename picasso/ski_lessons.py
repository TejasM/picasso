...
__author__ = 'tmehta'
emails = {"Snowhawks Raven Ski and Snowboard School": "toronto@snowhawks.com"}

base_url = 'http://www.helpwevegotkids.com'
url = 'http://www.helpwevegotkids.com/toronto/listings/sports/skiing--snowboarding?page='
...
golf = Tag.objects.get_or_create(tag_name='Ski', parent_tag=tag[0])
snowboard = Tag.objects.get_or_create(tag_name='Snowboard', parent_tag=tag[0])
...
for i in range(1, 2):
    ...
    for listing in listings:
        ...
        if Listing.objects.filter(scraped_url=scraped_url).count() == 0:
            ...
            l.tags = [tag[0].id, golf[0].id, snowboard[0].id]
            l.save()