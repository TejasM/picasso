...
url = 'http://www.helpwevegotkids.com/toronto/listings/sports/skating--hockey?page='
...
golf = Tag.objects.get_or_create(tag_name='Hockey', parent_tag=tag[0])
skating = Tag.objects.get_or_create(tag_name='Skating', parent_tag=tag[0])
...
for i in range(1, 3):
    ...
    for listing in listings:
        ...
        if Listing.objects.filter(scraped_url=scraped_url).count() == 0:
            ...
            l.tags = [tag[0].id, golf[0].id, skating[0].id]
            l.save()